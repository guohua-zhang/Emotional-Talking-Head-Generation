...
from manipulator.data.train_dataset import (InputFetcher, PairedInputFetcher,
...
def compute_d_loss(nets, opt, x_real, y_org, y_trg, z_trg=None, x_ref=None, x_gt=None):
    assert (z_trg is None) != (x_ref is None)
    # with real images
    # x_real.requires_grad_()

    if opt.paired and x_gt is not None:
        out = nets.discriminator(x_gt, y_trg)
...
def compute_g_loss(nets, opt, x_real, y_org, y_trg, z_trg=None, x_ref=None, x_gt=None):
    assert (z_trg is None) != (x_ref is None)
    # adversarial loss
    if z_trg is not None:
        s_trg = nets.mapping_network(z_trg, y_trg)
    ...
    if x_gt is not None:
        loss_paired = F.mse_loss(x_fake[:, :, 1:], x_gt[:, :, 1:]) * opt.lambda_paired
    else:
...
if __name__ == '__main__':
    ...
    fetcher_cls = PairedInputFetcher if opt.paired else InputFetcher
    fetcher = fetcher_cls(loader_src, loader_ref, opt.latent_dim)
    fetcher_val = fetcher_cls(loader_src_val, loader_ref, opt.latent_dim)
    ...
    for epoch in range(0 if opt.finetune else opt.resume_epoch, opt.niter):
        ...
        for i in range(len(loader_src)):
            ...
            d_loss, d_losses_ref = compute_d_loss(
                nets, opt, x_real, y_org, y_trg, x_ref=x_ref, x_gt=x_gt)
            reset_grad()
            d_loss.backward()
            optims.discriminator.step()

            # train the generator (and F, E)
            # g_loss, g_losses_latent = compute_g_loss(nets, opt, x_real, y_org, y_trg, z_trg=z_trg, x_gt=x_gt)
            # reset_grad()
            # g_loss.backward()
