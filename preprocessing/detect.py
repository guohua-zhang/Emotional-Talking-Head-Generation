...
def save_images(images, folder, split, start_i, args):
    transpose = folder.startswith('images')
    for i in range(len(images)):
        fid = i if split else i + start_i
        n_frame = "{:06d}".format(fid)
        save_dir = os.path.join(args.celeb, folder)
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        save_image(images[i], os.path.join(save_dir, n_frame + '.png'), transpose=transpose)
...
def detect_and_save_faces(detector, mp4_path, split, start_i, args):
    ...
    video_name = os.path.basename(mp4_path).rsplit('.', 1)[0]
    sub_folder = '/' + video_name if split else ''
    for i in tqdm(range(n_frames)):
        ...
        # else, detect faces in sequence and create new list
            ...
            save_images(tensor2npimage(face_images), 'images'+sub_folder, split, start_i, args)

            if args.save_full_frames:
                save_images(images, 'full_frames'+sub_folder, split, start_i, args)
...
def main():
    ...
    if os.path.isdir(images_dir):
        if args.save_full_frames:
            if os.path.isdir(os.path.join(args.celeb, 'full_frames')):
