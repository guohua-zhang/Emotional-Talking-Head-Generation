...
def m4a_to_wav(audio_path):
    cmd = 'ffmpeg -i {} -acodec pcm_s16le -ac 2 -ar 44100 {}'.format(audio_path, audio_path.replace('.m4a', '.wav'))
    try:
        subprocess.check_output(shlex.split(cmd), stderr=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        raise RuntimeError('ffpmeg returned non-zero status: {}'.format(e.stderr))
    except OSError as e:
        raise OSError(e.errno, 'ffpmeg not found.')
...
def recognize(args, model, audio_path):
    ...
    if args.extended:
        out = metadata_to_string(model.sttWithMetadata(audio, 1).transcripts[0])
    elif args.json:
        out = metadata_json_output(model.sttWithMetadata(audio, args.candidate_transcripts))
    else:
        out = model.stt(audio)
...
def main():
    ...
    for file in tqdm(audio_files):
        wav_file = file.replace('.m4a', '.wav')
        if not os.path.exists(wav_file):
            m4a_to_wav(file)
        text = recognize(args, ds, wav_file)
        results.append(wav_file.rsplit('.', 1)[0] + ' ' + text)

    with open(os.path.join(args.audio, 'texts.txt'), 'w') as f:
        f.write('\n'.join(results))
