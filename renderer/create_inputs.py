...
def read_DECA_params(path, device = 'cuda'):
    pkl_files = sorted(glob(os.path.join(path, '**/*.pkl'), recursive=True))
    params = []
    for p in pkl_files:
        with open(p, 'rb') as f:
            param = pickle.load(f)
...
def read_eye_landmarks(path):
    txt_files = sorted(glob(os.path.join(path, '**/*.txt'), recursive=True))
    #[os.path.join(path, txt) for txt in sorted(os.listdir(path))]
    eye_landmarks_left = []
    eye_landmarks_right = []
    for f in txt_files:
        if os.path.exists(f):
...
def main():
    ...
    for i, (src_codedict, trg_codedict, pth) in tqdm(enumerate(zip(src_codedicts, trg_codedicts, paths)), total=len(src_codedicts)):
        ...
        if not args.no_align:
            ...
            imwrite(nmfc_pth.replace('/nmfcs', '/nmfcs_aligned'), nmfc_image_a)
        ...
        if not args.no_eye_gaze:
            ...
            os.makedirs(os.path.dirname(eye_lnds_pth), exist_ok=True)
            np.savetxt(eye_lnds_pth, eye_lnds)

            if not args.no_align:
                eye_lnds_a = transform_points(eye_lnds, mat)
                eye_lnds_pth = eye_lnds_pth.replace('/eye_landmarks', '/eye_landmarks_aligned')
                os.makedirs(os.path.dirname(eye_lnds_pth), exist_ok=True)
                np.savetxt(eye_lnds_pth, eye_lnds_a)
    print('DONE!')
