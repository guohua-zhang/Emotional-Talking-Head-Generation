...
from renderer.util import util
...
class Head2HeadModelD(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if (opt.continue_train or opt.load_pretrain):
            ...
            if not opt.no_mouth_D:
                self.load_network(self.netDm, 'Dm',
                                  opt.which_epoch, opt.load_pretrain)
            if opt.use_eyes_D:
                self.load_network(self.netDe, 'De',
                                  opt.which_epoch, opt.load_pretrain)
        ...
        if opt.isTrain and hasattr(opt, 'lr'):
            self.old_lr = opt.lr
    ...
    def forward(self, tensors_list, mouth_centers=None, eyes_centers=None):
        ...
        loss_G_VGG = (self.criterionVGG(fake_B, real_B) *
                      lambda_feat) if not self.opt.no_vgg else torch.zeros(1, 1).cuda()
        # GAN loss for Generator
        loss_D_real, loss_D_fake, loss_G_GAN, loss_G_GAN_Feat = self.compute_D_losses(
            self.netD, real_A, real_B, fake_B)

        loss_list = [loss_G_VGG, loss_G_GAN,
                     loss_G_GAN_Feat, loss_D_real, loss_D_fake]

        if not self.opt.no_mouth_D:
            # Extract mouth region around the center.
            real_A_mouth, real_B_mouth, fake_B_mouth = util.get_ROI(
                [real_A, real_B, fake_B], mouth_centers, self.opt)
            # Losses for mouth discriminator
            loss_Dm_real, loss_Dm_fake, loss_Gm_GAN, loss_Gm_GAN_Feat = self.compute_D_losses(
                self.netDm, real_A_mouth, real_B_mouth, fake_B_mouth)
            mouth_weight = 1
            loss_Gm_GAN *= mouth_weight
            loss_Gm_GAN_Feat *= mouth_weight
            loss_list += [loss_Gm_GAN, loss_Gm_GAN_Feat,
                          loss_Dm_real, loss_Dm_fake]
        if self.opt.use_eyes_D:
            # Extract eyes region around the center.
            real_A_eyes, real_B_eyes, fake_B_eyes = util.get_ROI(
                [real_A, real_B, fake_B], eyes_centers, self.opt)
            # Losses for eyes discriminator
            loss_De_real, loss_De_fake, loss_Ge_GAN, loss_Ge_GAN_Feat = self.compute_D_losses(
                self.netDe, real_A_eyes, real_B_eyes, fake_B_eyes)
            eyes_weight = 1
            loss_Ge_GAN *= eyes_weight
            loss_Ge_GAN_Feat *= eyes_weight
            loss_list += [loss_Ge_GAN, loss_Ge_GAN_Feat,
                          loss_De_real, loss_De_fake]
...
class Head2HeadModelG(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(netG_input_nc, opt.output_nc, prev_output_nc,
                                      opt.ngf, opt.n_downsample_G, opt.norm, self.gpu_ids, opt)

        # load networks
        if not self.isTrain or opt.continue_train or opt.load_pretrain:
            self.load_network(
                self.netG, 'G', opt.which_epoch, opt.load_pretrain)
        ...
        if self.isTrain:
            # Optimizer for G
            self.n_frames_load = min(
                self.opt.max_frames_per_gpu, self.opt.n_frames_total)
    ...
    def forward(self, input_A, input_B, fake_B_prev):
        ...
        if is_first_frame:
            if self.opt.no_first_img:
                fake_B_prev = Variable(self.Tensor(
                    self.bs, self.n_frames_G-1, self.opt.output_nc, self.height, self.width).zero_())
            else:
        ...
        for t in range(self.n_frames_load):
            ...
            real_A_reshaped = real_A[:, t:t +
                                     self.n_frames_G, ...].view(self.bs, -1, h, w).cuda(gpu_id)

            fake_B_prevs = fake_B_prev[:, t:t +
                                       self.n_frames_G-1, ...].cuda(gpu_id)
            ...
            fake_B_prev = self.concatenate_tensors(
                [fake_B_prev, fake_B.unsqueeze(1).cuda(gpu_id)], dim=1)
        ...
        if tensors[0] is not None and tensors[1] is not None:
            if isinstance(tensors[0], list):
                ...
                for i in range(len(tensors[0])):
                    tensors_cat.append(self.concatenate_tensors(
